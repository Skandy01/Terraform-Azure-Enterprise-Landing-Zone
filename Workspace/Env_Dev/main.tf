...
module "mssql" {
  ...
  depends_on = [module.RG]
  source     = "../../Generic_Infra_Modules/Azure_SQL_Server"
...
module "sql_database" {
  ...
  source     = "../../Generic_Infra_Modules/Azure_SQL_Database"
  ...
  sql_dbs = {
    ...
    (each.key) = merge(each.value, {
    server_id = data.azurerm_mssql_server.mssql_id[each.value.server_key].id })
  }
}
...
module "container_registry" {
  ...
  source = "../../Generic_Infra_Modules/Azure_Container_Registry"
  acr    = var.acr
}
...
module "k8_cluster" {
  ...
  source = "../../Generic_Infra_Modules/Azure_kubernetes_Cluster"
  aks = var.aks
}
